...
class matterNode {
    ...
    constructor(server: any, roomId: string, obj: measurements) {
        ...
        console.log(this.obj.divHeight/2)
        this.ball = Bodies.circle(-155, this.obj.divHeight/2, this.obj.ball.radius, { label: "ball", restitution: 1.1, friction: 0, frictionAir: 0, density: 10 });
                    ...
                    if (this.ready) {
                        ...
                        if (this.ball.position.y < 0) {
                            ...
                            if (this.score.right >= 2){
                                this.ready = false
                                this.server.to(this.roomId).emit("gameOver", { gameOver: "right" });
                            }
                            else
                                setTimeout(() => {    // after seconds launch the ball again
                                    Body.setVelocity(this.ball, { x: 5, y: 6 });
                        ...
                        console.log(this.ball.position.y)
                            ...
                            if (this.score.left >= 2){
                                this.ready = false
                                this.server.to(this.roomId).emit("gameOver", { gameOver: "left" });
                            }
                            else
                                setTimeout(() => {
                                    Body.setVelocity(this.ball, { x: 5, y: -6 });
                        ...
                        this.server.to(this.roomId).emit('gameOver', { gameOver: true });
                }
                else
                    this.server.to(this.roomId).emit('ready', { msg: "awaiting second player to connect..." });
